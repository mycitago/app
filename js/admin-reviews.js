...
function renderReviews(){
  ...
  if(!list.length){host.innerHTML='<div class="ct-empty reviews-empty"><strong>Aún no hay reseñas</strong><span>Las reseñas verificadas de MyCitaGo aparecerán aquí aunque Google no esté conectado.</span></div>';return}
  ...
  list.forEach(x=>{
    ...
    a.innerHTML=`<div class="review-head"><div><strong>${safe(x.reviewer_name||'Cliente')}</strong><span class="source-label">${safe(sourceLabel(x))}</span>${verified}</div><span class="review-stars">${'★'.repeat(Number(x.star_rating||0))}${'☆'.repeat(Math.max(0,5-Number(x.star_rating||0)))}</span></div>${x.comment?`<p>${safe(x.comment)}</p>`:''}${x.reply_comment?`<div class="review-reply">Tu respuesta: ${safe(x.reply_comment)}</div>`:''}<div class="review-actions"><button class="ct-btn ct-btn-secondary" type="button">${x.reply_comment?'Editar respuesta':'Responder'}</button></div>`;
    a.querySelector('button').onclick=()=>reply(x);host.appendChild(a);
  });
}
...
async function loadReviews(){
  const {data,error}=await supabaseClient.from('reviews')
    .select('id,appointment_id,verified,source,rating,comment,reviewer_name,reply_text,external_review_id,external_created_at,created_at')
    .eq('business_id',reviewBiz.id).eq('status','published').order('created_at',{ascending:false});
  if(error){console.error(error);reviewRows=[]}else reviewRows=(data||[]).map(normalizeInternal);
  const total=reviewRows.length;
  R('review-total').textContent=total;
  R('review-rating').textContent=total?(reviewRows.reduce((a,x)=>a+Number(x.star_rating||0),0)/total).toFixed(1)+' ★':'–';
  const verifiedCount=reviewRows.filter(x=>x.source==='internal'&&x.verified).length;
